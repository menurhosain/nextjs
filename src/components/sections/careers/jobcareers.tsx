...
const fallbackListings: JobListing[] = [
    { job_status: "Full Time", job_title: "Project Manager Consulting", btn_label: "View Open Positions", btn_link: "#" },
    { job_status: "Full Time", job_title: "Project Manager Consulting", btn_label: "View Open Positions", btn_link: "#" },
    { job_status: "Full Time", job_title: "Project Manager Consulting", btn_label: "View Open Positions", btn_link: "#" },
    { job_status: "Full Time", job_title: "Project Manager Consulting", btn_label: "View Open Positions", btn_link: "#" },
];

export default function JobCareers({ job_section_title, job_section_description, job_board_image, job_listings }: Props) {
    const [hoveredIndex, setHoveredIndex] = useState<number | null>(null);
    const listings = job_listings && job_listings.length > 0 ? job_listings : fallbackListings;
                        ...
                        <div className="w-full xl:w-[605px]">
                            <p className="text-[16px] text-sah-gray-2 font-medium leading-[28px]">
                                {job_section_description ||
                                    "With us, your career is always moving forward. You'll grow through continuous learning, guided by mentorship at every stage, while collaborating with diverse teams to broaden your skills, knowledge, and experience."}
                            </p>
                        </div>
                    </div>
                        ...
                        <div className="w-full xl:w-[65%] 2xl:[54%] flex flex-col divide-y divide-sah-gray-4 border-t border-t-sah-gray-4 border-b border-b-sah-gray-4">
                            {listings.map((item, index) => (
                                <div
                                    ...
                                    className={`group flex max-[640px]:flex-col max-[640px]:text-center max-[640px]:gap-3 items-center justify-between py-[27px] px-0 transition-all duration-300 cursor-pointer ${hoveredIndex === index ? "bg-white max-[768px]:bg-transparent -mx-3 max-[768px]:-mx-0 px-6 max-[768px]:px-0" : ""}`}
                                >
                                    <span className="text-[17px] text-sah-dark-2 px-[10px] py-[4px] font-regular rounded-[30px] bg-sah-white transition-all duration-300 group-hover:bg-sah-light-4">
                                        {item.job_status || "Full Time"}
                                    </span>

                                    <span className={`flex-1 text-[18px] 2xl:text-[20px] font-semibold text-gray-900 sm:ml-[30px] transition-colors duration-300 ${hoveredIndex === index ? "text-black" : ""}`}>
                                        {item.job_title}
                                    </span>
                                    ...
                                    <div className="flex items-center gap-[10px] sm:ml-4 shrink-0">
                                        <a
                                            href={item.btn_link || "#"}
                                            className={`flex items-center gap-2 text-[14px] sm:text-[16px] font-medium pl-2 sm:pl-[30px] pr-[10px] py-2 rounded-full transition-all duration-300 ${hoveredIndex === index ? "bg-sah-dark-2 text-white scale-105" : "bg-white text-gray-700"}`}
                                        >
                                            {item.btn_label || "View Open Positions"}
                                            <span className={`flex items-center justify-center w-[28px] h-[28px] rounded-full transition-all duration-300 ${hoveredIndex === index ? "bg-sah-red text-white" : "bg-sah-dark-2 text-white"}`}>
