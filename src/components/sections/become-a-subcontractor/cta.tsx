...
export default function Cta({ bg, subtitle, title, description, button_label, button_link }: Props) {
    return (
        <section className="section-padding pt-[80px] lg:pt-[150px] xl:pt-[300px] 2xl:pt-[290px] relative">
            <video autoPlay muted loop playsInline className="absolute inset-0 h-full w-full object-cover z-[-1]">
                <source src={bg || "/career.mp4"} type="video/mp4" />
            </video>
            ...
            <div className="container h-full flex items-end max-[1536px]:!px-0">
                <div className="w-full lg:w-[700px] xl:w-[900px] 2xl:w-[920px] p-[30px] md:p-[50px] xl:p-[100px] bg-[url('/career-content-bg.jpg')] bg-cover bg-top bg-no-repeat rounded-tr-[10px] rounded-tl-[10px]">
                    <Section_Title
                        subtitle={subtitle || "Life at Sah"}
                        title={title || "Career with SAH"}
                        description={description || "Saif Salim Essa Al Harasi & Co. LLC. (SAH) is a renowned construction company based in the Sultanate of Oman. With a rich legacy spanning several decades, SAH has established itself as a trusted name in the construction industry, delivering exceptional projects"}
                    ...
                    <div className="mt-[40px] w-max">
                        <ButtonModern variant="solid" link={button_link || "#"} label={button_label || "You Like to Build?"} class_name="max-[640px]:px-3" />
                    </div>
                </div>
            </div>
        </section>
