            ...
            <div className="min-h-screen flex items-center justify-center bg-gray-50 px-4">
                <div className="w-full max-w-md bg-white rounded-2xl shadow-lg p-8 space-y-6">
                    <div className="flex items-center gap-4">
                        {pictureUrl ? (
                            <img src={pictureUrl} alt="Profile picture" className="w-16 h-16 rounded-full object-cover" />
                        ) : (
                            <div className="w-16 h-16 rounded-full bg-gray-200 flex items-center justify-center text-2xl font-bold text-gray-500">{initials}</div>
                        )}
                        <div>
                            <h1 className="text-2xl font-bold text-gray-900">{firstName && lastName ? `${firstName} ${lastName}` : (username ?? "My Profile")}</h1>
                            {type && <span className="text-xs font-medium bg-gray-100 text-gray-600 px-2 py-0.5 rounded-full capitalize">{type}</span>}
                        </div>
                    </div>
                    ...
                    <a href="/profile/edit" className="block w-full text-center text-sm font-medium bg-gray-900 text-white rounded-lg py-2.5 hover:bg-gray-700 transition-colors">
                        Edit Profile
                    </a>
                </div>
            </div>
